...
describe User do
  ...
  describe 'user should save with correct validations' do 
    ...
    it "user should not save without a valid password" do
      user = User.new(:name => 'Pete', :email => 'test@test.com', :password => 'pass', :password_confirmation => 'pass')
      user.save
      user.errors.should include :password
    end

    it "user should not save if the passwords don't match" do
      user = User.new(:name => 'Jen', :email => 'test@test.com', :password => 'password', :password_confirmation => 'password1')
      user.save
      user.errors.should include :password
    end

    it "user should not save without a valid email address" do
      user = User.new(:name => 'Mum', :email => 'test', :password => 'password', :password_confirmation => 'password')
      user.save
      user.errors.should include :email
    end

    it "user should not save with a non-unique email address" do
      User.create(:name => 'Dad', :email => 'test@test.com', :password => 'password', :password_confirmation => 'password')
    ...
    it "user should save with valid credentials" do
      user = User.new(:name => 'Adam', :email => 'test@test.com', :password => 'password', :password_confirmation => 'password')
      user.save
