...
describe PhotosController do
  ...
  describe "when not logged in" do
    ...
    before(:each) do
      ...
      @photo = Factory(:photo, :institution_id => @church.id)
    end
  ...
  describe "when logged in" do
    ...
    it "should correctly redirect and increase the photo count when creating a photo" do
      lambda do
        post 'create', :photo => Factory.attributes_for(:photo).merge(:institution_id => @church.id)
        response.should be_redirect
    ...
    it "should handle the update of an photo" do
      ...
      put 'update', :id => @photo.id, :photo => Factory.attributes_for(:photo).merge(:institution_id => @church.id), :church_id => @church.slug
      response.should be_redirect
      response.location.should include photos_path(@church)
    end
    ...
    it "should display an error message and render the edit action if photo is not valid on update" do
      ...
      put 'update', :id => @photo.id, :church_id => @church.slug, :photo => Factory.attributes_for(:photo).merge(:institution_id => @church.id)
      flash[:error].should == 'There was an error updating the photo'
      response.should render_template('edit')
    end
    
    it "should change the order of the photos" do
      @photo1 = Factory(:photo, :institution_id => @church.id)
      @photo2 = Factory(:photo, :institution_id => @church.id)
      post 'order_photos', :photo => [@photo2.id.to_s, @photo1.id.to_s]
      @church.photos.asc(:position).first.should eql @photo2
    end
