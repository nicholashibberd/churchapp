...
describe EventsController do
  ...
  describe "when not logged in" do
    ...
    it "should restrict access to the edit action" do
      get 'edit', :church_id => @institution.slug, :id => @event.id
      response.should be_redirect
      response.location.should == 'http://www.example.com/signin'
    end
    
    it "should not restrict access to the show action" do
      get 'show', :church_id => @institution.slug, :id => @event.id
  ...
  describe "when logged in" do
    ...
    before(:each) do
      ...
      @event = Factory(:event, :church_id => @institution.id)
    end
    ...
    it "should correctly redirect and increase the event count when creating an event" do
      lambda do
        post 'create', :event => Factory.attributes_for(:event), :church_id => @institution.slug
        response.should be_redirect
        response.location.should include events_path(@institution)
    ...
    it "should create multiple events when event series is specified" do
      ...
      post 'create', :event => Factory.attributes_for(:event).merge(:period => 'Weekly'), :church_id => @institution.slug
    ...
    it "should correctly redirect and handle the update of an event when valid" do
      ...
      put 'update', :church_id => @institution.slug, :id => @event.id
      response.should be_redirect
      response.location.should include events_path(@institution)
    end
    ...
    it "should correctly redirect and decrease the event count when deleting an event" do
      lambda do
        delete 'destroy', :church_id => @institution.slug, :id => @event.id
        response.should be_redirect
        response.location.should include events_path(@institution)
      end.should change(Event, :count).by(-1)
    end
